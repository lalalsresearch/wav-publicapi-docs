...
Webhooks allow wav to notify your application the moment a task is completed. You’ll get all the necessary data including the audio URL without needing to constantly poll our API.
...
2. wav will **send a POST request** to that URL when the task is completed.
...
```json
{
  ...
  "audio_url": "https://cdn1.wav.com/audiofile.mp3",
...
When you initiate a conversion request, wav will immediately respond with a **preliminary response** that includes:
...
Set it once, and let wav notify you when the magic happens. 🎶
