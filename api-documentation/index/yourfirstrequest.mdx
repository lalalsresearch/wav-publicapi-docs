...
Welcome to your **first experience** with the wav API, where creativity meets technology. Browse the wide range of features available for you through our API. 
...
1. [Activate](https://wav.com/api-dashboard) your API Key.
2. Go to [Documentation](https://docs.wav.com/api-documentation/index/introduction).
3. Inside features, you can click Try-it button to open a live [playground](https://docs.wav.com/api-documentation/conversions/musicaiv2?playground=open).
4. You can also use the multi-language code samples provided for each feature to run it locally.
5. To get result: Set the [**webhook_url**](https://docs.wav.com/api-documentation/index/webhook) to an endpoint on your server or retrieve the result through our helper endpoint [**get conversion by id**](https://docs.wav.com/api-documentation/endpoint/getById) instead.
...
```http
POST https://api.wav.com/api/public/v1/MusicAI
```
...
```bash
curl -X 'POST' \
'https://api.wav.com/api/public/v1/MusicAI' \
...
> Replace `<api_key>` with your actual API Key from your WAV dashboard.
...
> 🔗 Set the `webhook_url` to an endpoint on your server — this is where WAV will notify you once the generation is done.

> **Tip:** If you can’t use a webhook (e.g., code running on a user device), you can retrieve the result through our helper endpoint [get conversion by id](/api-documentation/endpoint/getById) instead.
...
You just generated your **first AI song** using the wav API. Now go make something amazing ✨
